...
@lt_bp.route('/lt/<log_type>/update/<int:log_id>', methods=['POST'])
def update_log(log_type, log_id):
    if log_type in config['log_types']:
        updated_log_data = request.form.to_dict()
        logs = read_logs(log_type)

        for log in logs:
            if log.get('id') == log_id:
                log.update(updated_log_data)
                break
...
@lt_bp.route('/lt/<log_type>/delete/<int:log_id>', methods=['POST'])
